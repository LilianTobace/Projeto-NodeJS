...
function migrateForLoop(elNode, context) {
  ...
  const isLegacyTemplate = context.meta.legacy || context.hasMigrationFlag("legacyWidgetAttrsWithoutBind");
  ...
  switch (parsed.loopType) {
    case "ForEach":
      {
        ...
        if (parsed.iterator) {
          if (parsed.statusVarName) {
            ...
            parsed.in = builder.functionCall(builder.memberExpression(parsed.iterator, builder.identifier("bind")), [builder.literalNull(), parsed.in]);
          } else {
            // Otherwise we can run the iterator early to build an array for use with a standard `<for of=array>`.
            const iteratorName = parsed.iterator.type === "Identifier" ? parsed.iterator : builder.identifier("iterator");
            const listName = parsed.in.type === "Identifier" ? parsed.in : builder.identifier("list");
            const iteratorResultName = builder.identifier(camelCase(`${iteratorName.name}-${listName.name}`));
            const itemName = builder.identifier("item");
            elNode.insertSiblingBefore(builder.scriptlet({
              value: printJS(builder.vars([builder.variableDeclarator(iteratorResultName, builder.arrayExpression([]))]), context)
            }));

            elNode.insertSiblingBefore(builder.scriptlet({
              value: printJS(builder.functionCall(parsed.iterator, [parsed.in, builder.functionDeclaration(undefined, [itemName], [builder.functionCall(builder.memberExpression(iteratorResultName, builder.identifier("push")), [itemName])])]), context)
            }));
        ...
        if (!isLegacyGenerator) {
          if (parsed.separator) {
            needsParams = true;
            elNode.appendChild(builder.htmlElement("if", undefined, [builder.text(parsed.separator, false, true)], `${varNamePrefix}Index !== ${varNamePrefix}All.length - 1`));
          }

          if (needsParams) {
            elNode.params.push(builder.identifier(`${varNamePrefix}Index`), builder.identifier(`${varNamePrefix}All`));
          }
        }
    ...
    case "ForEachProp":
      if (parsed.statusVarName || parsed.separator) {
        ...
        elNode.argument = `${printJS(parsed.nameVarName, context)} in Object.keys(${printJS(parsed.in, context)}) |`;

        if (parsed.statusVarName) {
          elNode.argument += ` status-var=${printJS(parsed.statusVarName, context)}`;
        }
        ...
        elNode.prependChild(builder.scriptlet({
          value: printJS(builder.var(parsed.valueVarName, builder.memberExpression(parsed.in, parsed.nameVarName, true)), context)
        }));
    ...
    case "For":
      {
        ...
        if (!parts) {
          ...
          if (parsed.update) {
            elNode.appendChild(builder.scriptlet({
              value: printJS(parsed.update, context),
              block: true
            }));
          }

          elNode.replaceWith(builder.htmlElement("while", undefined, elNode.body, parsed.test ? printJS(parsed.test, context) : "true"));
          return;
        }
...
function normalizeParts(parsed, builder) {
  ...
  let step;
  if (update.type === "UpdateExpression" && update.argument.name === varName.name) {
    step = builder.literal(update.operator === "++" ? 1 : -1);
  } else if (update.type === "Assignment" && update.left.name === varName.name) {
    if (update.operator === "-=") {
      step = update.right;
...
function replaceStatusVarInBody(el, context, parsed) {
  ...
  const lengthIdentifier = builder.memberExpression(allIdentifer, builder.identifier("length"));
  const remaps = {
    getIndex: indexIdentifier,
    getLength: lengthIdentifier,
    isFirst: builder.binaryExpression(indexIdentifier, "===", builder.literal(0)),
    isLast: builder.binaryExpression(indexIdentifier, "===", builder.binaryExpression(lengthIdentifier, "-", builder.literal(1)))
  };
  ...
  const remapRegexp = new RegExp(`${statusVarName}\\.(${remapKeys.join("|")})\\(\\)`, "g");
  const remapReplacer = str => str.replace(remapRegexp, (_, key) => remapStrings[key]);
  const visitor = {
    enter(node) {
      if (node.type === "Text" && !node.isLiteral() || (node.type === "HtmlElement" || node.type === "HtmlAttribute") && node.argument) {
        ...
        if (callee.type === "MemberExpression" && callee.object.name === statusVarName && remapKeys.includes(callee.property.name)) {
